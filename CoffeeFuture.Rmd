...
*"If we have enough data, I know we can make predictions," Simons told a colleague.*[^7]
...
The first model in this project is the k-nearest neighbors (kNN) approach, used by Renaissance Technologies and presented at chapter 29.1 Motivation with k-nearest neighbors [@irizarry2019]. The multiple dimensions adaptability is a important feature, specialy in finance, to estimate $p(x_1, x_2)$:

$$
p(x_1, x_2) = \mbox{Pr}(Y=1 \mid X_1=x_1 , X_2 = x_2).
$$

As presented "for any point $(x1,x2)$ for which we want an estimate of $p(x1,x2)$, we look for the k nearest points to $(x1,x2)$ and then take an average of the 0s and 1s associated with these points. We refer to the set of points used to compute the average as the *neighborhood*." [@irizarry2019].
We going to use the knn3 function of caret package, with a $k=5$, to train the dataset. Then we show the first 5 rows of model by probability and class of decision.
...
The tree based method, presented in chapter 6 [@coqueret2020a], is our second model, and was choosen by efficient forecast, are easy to visualize and could model human decision. The goal of model is to minimize the dispersion inside each cluster ?
...
A matrix Factorization is the last approach to solve recommendation system problem presented in this The model improved substantially and we achieve the best result so far, we have our choice to train and test the dataset.
